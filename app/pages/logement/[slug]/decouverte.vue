...
<template>
  <div class="flex w-full flex-col gap-22 pb-22">
    ...
    <section class="w-full">
      ...
      <p class="m-0 text-xl leading-21 font-normal text-ls-muted">
    ...
    <section class="flex w-full flex-col gap-11" aria-labelledby="log-method-title">
      ...
      <div class="relative flex w-full items-start">
        <div
          aria-hidden="true"
          class="pointer-events-none absolute top-44 left-[60px] right-[60px] h-0 border-t border-dashed border-ls-step-line max-2xs:hidden"
        />
        <div
          v-for="step in steps"
          :key="step.id"
        ...
        >
          <span
            :class="[
              'mb-8 flex size-48 shrink-0 items-center justify-center rounded-full',
              step.bg,
            ]"
          >
          ...
          <p class="m-0 mt-2 w-full text-center text-base leading-15 font-medium text-ls-step-label">
            {{ $t(step.labelKey) }}
          </p>
        </div>
      </div>
    ...
    <!-- Engagements -->
    <aside class="box-border flex w-full items-start rounded-[16px] border border-ls-engage-border bg-ls-engage-bg p-20">
      <span class="mr-16 flex size-40 shrink-0 items-center justify-center rounded-full bg-ls-engage-icon">
        <img
      ...
      <div class="min-w-0 flex-1 pt-2">
        <h3 class="m-0 text-2xl leading-[22.5px] font-semibold text-ls-step-label">
          {{ $t('housing.intro.engageTitle') }}
        </h3>
        <p class="m-0 mt-4 text-xl leading-[19.25px] font-normal text-ls-engage-desc">
    ...
    <NuxtLink
      ...
      class="box-border flex w-full items-center justify-center rounded-[10px] bg-primary-cta px-24 py-16 text-center text-xl leading-[22.5px] font-semibold text-white no-underline"
    >
      {{ $t('housing.intro.cta') }}
    </NuxtLink>
  </div>
