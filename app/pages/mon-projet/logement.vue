...
const { steps, progressPercent } = useLogementData()
...
function updateLines() {
  ...
  // La maquette scinde à la cinquième pastille, la première « en attente ».
  const splitIdx = Math.min(4, markers.length - 1)
  ...
  <div class="page-mpl flex min-h-screen flex-1 flex-col bg-white">
    ...
    <div class="mpl-main flex flex-col pb-[var(--spacing-content-bottom)] box-border">
      ...
      <section class="mpl-progress mt-8 w-full rounded-xl border border-mpo-card-border bg-mpl-progress-bg px-19 py-25 shadow-2xs box-border" :aria-label="$t('projectHousing.progressLabel')">
        <div class="mpl-progress-top flex items-start justify-between gap-12">
          ...
          <div class="mpl-progress-pct flex shrink-0 flex-col items-end">
            <span class="mpl-progress-num text-5xl leading-30 font-bold text-mpo-pink">{{ progressPercent }}%</span>
        ...
        <div class="mpl-progress-bar mt-16 h-6 w-full overflow-hidden rounded-full bg-mpo-bar-bg" aria-hidden="true">
          <span class="mpl-progress-fill block h-full rounded-full bg-mpo-pink" :style="{ width: `${progressPercent}%` }" />
        </div>
      </section>
      ...
      <div v-show="activeTab === 'formule'" class="mpl-panel mt-24 w-full">
        <div ref="timelineRef" class="mpl-timeline relative w-full rounded-xl border border-mpo-card-border bg-white pt-21 pr-10 pb-21 pl-40 shadow-xs box-border">
          ...
          <div
            ...
            :class="[
              ...
              index === steps.length - 1 ? 'mpl-step--last pb-0' : '',
            ]"
          >
            <span
              :class="[
