...
const steps = [
  ...
  { icon: 'ic-paiement-step2', tone: 'bg-paiement-step-2', titleKey: 'confirmation.step2Title' },
  { icon: 'ic-paiement-step3', tone: 'bg-success', titleKey: 'confirmation.step3Title' },
  { icon: 'ic-paiement-step4', tone: 'bg-paiement-step-4', titleKey: 'confirmation.step4Title' },
  { icon: 'ic-lpp-step-visio', tone: 'bg-paiement-step-1', titleKey: 'confirmation.step5Title' },
] as const

/** Séparateurs entre 5 colonnes : 20 / 40 / 60 / 80 %. */
const stepDividers = [
  'left-[calc(20%-10px)]',
  'left-[calc(40%-10px)]',
  'left-[calc(60%-10px)]',
      ...
      <template v-if="confirmed">
        ...
        <div>
          ...
          <div class="relative flex min-h-150 items-start pt-10">
            <QIcon
              v-for="position in stepDividers"
              :key="position"
            ...
            <div v-for="(step, index) in steps" :key="step.titleKey" class="flex min-w-0 flex-1 flex-col items-center gap-11">
              <div class="flex w-full max-w-88 flex-col items-center gap-18">
                <QIcon :name="step.icon" :size="index === 4 ? 44 : 50" />
                <span
                  :class="[
      ...
      <div>
        <div class="relative flex min-h-91 items-center justify-between gap-8 rounded-xl bg-surface-2 p-9">
          <div class="flex min-w-0 flex-1 items-start gap-11 pr-120 max-[375.02px]:pr-0 max-[375.02px]:pb-44">
            ...
            <div class="min-w-0">
              <p class="m-0 text-md leading-20 font-bold text-text">{{ $t('confirmation.supportTitle') }}</p>
