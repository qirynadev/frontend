  ...
  <AppDesktopReglagesShell :title="$t('settingsLegal.title')" :intro="$t('settingsLegal.intro')">
    <aside class="flex items-start rounded-[16px] bg-[#f8f8fc] p-24">
      <span class="flex size-48 shrink-0 items-center justify-center rounded-full bg-white">
        <img :src="`${ICON}/ic-ril-trust.svg`" alt="" width="24" height="24" class="block size-24" loading="lazy" decoding="async">
      </span>
        ...
        >
          <span class="flex min-w-0 flex-1 items-start">
            <span v-if="row.tile" class="size-40 shrink-0 overflow-hidden">
              <img :src="`${ICON}/${row.icon}.svg`" alt="" width="40" height="40" class="block size-40" loading="lazy" decoding="async">
            </span>
            <span v-else class="flex size-40 shrink-0 items-center justify-center rounded-[12px] bg-[#f8f8fc]">
              <img :src="`${ICON}/${row.icon}.svg`" alt="" width="20" height="20" class="block size-20" loading="lazy" decoding="async">
            </span>
          ...
          <img :src="`${ICON}/ic-ril-chevron.svg`" alt="" width="20" height="20" class="ml-8 block size-20 shrink-0" loading="lazy" decoding="async">
        </component>
      </div>
    </section>
    ...
    >
      <span class="flex min-w-0 items-start">
        <span class="size-40 shrink-0 overflow-hidden">
          <img :src="`${ICON}/ic-ril-mail.svg`" alt="" width="40" height="40" class="block size-40" loading="lazy" decoding="async">
        </span>
        <span class="flex min-w-0 flex-1 flex-col pl-16">
      ...
      <img :src="`${ICON}/ic-ril-mail-chevron.svg`" alt="" width="20" height="20" class="ml-8 block size-20 shrink-0" loading="lazy" decoding="async">
    </a>
