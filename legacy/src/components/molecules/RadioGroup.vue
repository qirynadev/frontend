<script setup>
import { RadioGroup, RadioGroupLabel, RadioGroupDescription, RadioGroupOption } from "@headlessui/vue";

const model = defineModel({
  type: Boolean,
  required: true,
});

const props = defineProps({
  items: {
    type: Array,
    required: true,
  },
  value: {
    type: String,
    required: false,
    default: null,
  },
  label: {
    type: String,
    required: false,
    default: null,
  },
});
</script>

<template>
  <div class="w-full">
    <div class="mx-auto w-full max-w-md">
      <RadioGroup v-model="model">
        <RadioGroupLabel class="sr-only">Disponibilité</RadioGroupLabel>
        <div class="space-y-2">
          <RadioGroupOption
            as="template"
            v-for="item in props.items"
            :key="item[props.value]"
            :value="item[props.value]"
            v-slot="{ active, checked }"
          >
            <div
              :class="[
                active ? 'ring-2 ring-white/60 ring-offset-2 ring-offset-sky-300' : '',
                checked ? 'bg-sky-900/75 text-white ' : 'bg-surface ',
              ]"
              class="relative flex cursor-pointer border rounded-lg px-5 py-4 shadow-md focus:outline-none"
            >
              <div class="flex w-full items-center justify-between">
                <div class="flex items-center">
                  <div class="text-sm">
                    <RadioGroupLabel as="p" :class="checked ? 'text-white' : 'text-gray-900'" class="font-medium">
                      {{ item[props.label] }}
                    </RadioGroupLabel>
                    <RadioGroupDescription
                      as="span"
                      :class="checked ? 'text-sky-100' : 'text-ink-muted'"
                      class="inline"
                      v-if="item.description"
                    >
                      <span>{{ item.description }}</span>
                    </RadioGroupDescription>
                  </div>
                </div>
                <div v-show="checked" class="shrink-0 text-white">
                  <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none">
                    <circle cx="12" cy="12" r="12" fill="#fff" fill-opacity="0.2" />
                    <path
                      d="M7 13l3 3 7-7"
                      stroke="#fff"
                      stroke-width="1.5"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                  </svg>
                </div>
              </div>
            </div>
          </RadioGroupOption>
        </div>
      </RadioGroup>
    </div>
  </div>
</template>
