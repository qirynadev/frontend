...
import { useAppStore, useAuthStore, useMbaStore, usePaymentStore } from "@/stores";
...
import type { MentorType, OfferType, SchoolType } from "@/constants/constant.type";
...
const selectedMentor: ComputedRef<MentorType | null> = computed(() => mbaStore.selectedMentor);
const selectedOffer: ComputedRef<OfferType | null> = computed(() => mbaStore.selectedFormula);
const selectedMenuSlug: ComputedRef<string | null> = computed(() => appStore.selectedMenuSlug);

const userToken = computed(() => authStore.token);

const handleClickMentor = async () => {
  const orderData = {
...
onBeforeMount(async () => {
  if (selectedMentor) {
    ...
    const mentor = await appStore.fetchMentorById(selectedMentor.value?.id as string);
    mbaStore.selectedMentor = mentor;
          ...
          <div class="mentor-info-container">
            ...
            <div class="join_section mt-2 hidden md:flex justify-between items-center">
              ...
              <ButtonGeneral
                ...
                :title="!userToken ? $t('select') : $t('pay')"
                @pressed="handleClickMentor"
                :isLoading="isLoading"
              />
            </div>

            <button
              @click.prevent="handleClickMentor"
              type="button"
      ...
      <div class="right-part hidden lg:block">
        <div class="offer-wrapper">
          <SingleOfferItem
            ...
            :withPrice="false"
            :btnText="(selectedOffer as OfferType)?.amount + ' €'"
