...
import iconFacebookStacked from "@/assets/images/login-mobile/icon-facebook.svg";
...
const getErrorMessage = (error: any): string => {
  ...
  if (error?.message?.includes("not found")) {
    return t("oauth.errors.facebook.login-failed");
  }
...
const socialLogin = async (response: any) => {
  try {
    ...
    if (error?.type === "no-token") {
      ...
      errorMessage = t("oauth.errors.facebook.login-failed");
    ...
    ElNotification({
      title: t("oauth.errors.title.facebook-login"),
      type: "error",
  ...
  <div class="social-connection flex items-center justify-center" :class="{ 'min-w-0 flex-1': variant === 'stacked' }">
    ...
    >
      ...
      >
        ...
        <img v-else :src="iconFacebookStacked" class="h-4 w-4 flex-none" alt="" />
        <span class="min-w-0 flex-1 truncate text-center text-[11px] font-semibold text-ink">Facebook</span>
      </button>
      <button v-else class="flex items-center justify-center gap-3" aria-label="Facebook" @click="fbLogin.initFBLogin">
...
<style scoped>
/* HFaceBookLogin enveloppe son slot dans un <div> sans classe, ce qui casse la
   chaîne flex-1/min-w-0 entre .social-connection et le <button> réel — le bouton
   Facebook se retrouve alors dimensionné à son contenu au lieu de s'étirer comme
   les boutons Google/LinkedIn. On neutralise ce wrapper hors du flow flex. */
.social-connection > :deep(div) {
  display: contents;
}
</style>
