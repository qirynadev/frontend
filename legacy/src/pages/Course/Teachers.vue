...
const sectionTitle = computed(() => selectedCourse.value?.language ?? "");
...
const handleClickTeacher = (teacher: TeacherType) => {
  courseStore.setSelectedTeacher(teacher);
  router.push(i18nRoute({ name: "teacher-presentation", params: { teacherslug: teacher.slug } }));
};

onBeforeMount(() => {
  router.replace(i18nRoute({ name: "courses", params: { slug: route.params.slug as string } }));
});
</script>
...
.gift-callout { display: flex; align-items: center; gap: 12px; background: #fff; border: 1px solid $line; border-radius: 16px; padding: 14px 16px; box-shadow: 0 6px 24px rgba(20,30,60,.06); max-width: 246px; align-self: center; }
...
.help-card .ic { width: 44px; height: 44px; flex: 0 0 44px; border-radius: 50%; background: #fff; display: grid; place-items: center; box-shadow: 0 1px 2px rgba(20,30,60,.05); font-size: 20px; }
...
.pager button { width: 40px; height: 40px; border-radius: 50%; border: 1px solid $line; background: #fff; color: $navy; font-weight: 600; font-size: 15px; display: grid; place-items: center; transition: all .15s; cursor: pointer; }
